...
class Context
{
    ...
    private $m_nInstanceSystemActionTimestamp = NULL;   //Periodically update
    ...
    public static function getInstance($forceReset=FALSE, $bSystemDrivenAction=FALSE)
    {
        ...
        if(isset($_SESSION[CONST_NM_RAPTOR_CONTEXT]) && !$bSessionResetFlagDetected)
        {
            ...
            error_log('CONTEXTgetInstance::WORKFLOWDEBUG>>>NO EXISTING SESSION!!! Not using an existing session: bSessionResetFlagDetected='.$bSessionResetFlagDetected . ' from '. $_SERVER['REMOTE_ADDR'] 
    ...
    function setSelectedTrackingID($sTrackingID, $bClearPersonalBatchStack=FALSE)
    {
        ...
        //$debugmsg = 'LOOK setSelectedTrackingID>>>>'.$this;
        //error_log($debugmsg);
     ...
     */
    private function serializeNow($logMsg = '', $bSystemDrivenAction=TRUE)
    {
        if($bSystemDrivenAction)
        {
        ...
        if (!isset($_SESSION['CREATED']) || (time() - $_SESSION['CREATED']) > $SESSION_REFRESH_DELAY) 
        {
            // session started more than 30 minutes ago
            error_log('WORKFLOWDEBUG>>>Session key timeout reached so generated new key for uid='.$this->getUID());
