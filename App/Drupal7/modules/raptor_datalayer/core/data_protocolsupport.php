...
*/          if(in_array(strtoupper($vital['name']), $tempLabels)){ // Temp
    ...
    function getProcedureLabsDetail()
    {
        ...
        if($aCachedResult !== null)
        {
            ...
            global $user;
            error_log('LOOK cached result from getProcedureLabsDetail for user='.$user->name
                    .' has '.count($aCachedResult). ' lab results...'
                    . '\n\t... '.print_r($aCachedResult, TRUE));
        ...
        $proc = strtoupper($this->m_aDashboardMap['Procedure']); // Procedure is alialsed as RqstdStdy in method of WorklistData object
        //do not echo --- else shows up on page! echo $proc;
        $isProc = TRUE; //strpos($proc, "PROCEDURE") !== FALSE;

        $nMAX_NUMBER_ROWS=99999;  //No filter for labs tab as per DrM repeated request on 3/7/2012
...
//START TEMP LOGGING
global $user;
error_log('LOOK getDisplayLabs in getProcedureLabsDetail for user='.$user->name
       .' is'.print_r($allLabs, TRUE));
//END TEMP LOGGING
        ...
        {
             global $user;
             error_log('LOOK all labs row in getProcedureLabsDetail for user='.$user->name
                    .' is'.print_r($row, TRUE));
        ...
        //START TEMPORARY DIAGNOSTIC LOGGING
        global $user;
        error_log('LOOK result from getProcedureLabsDetail for user='.$user->name
                .' and patient='.$patientInfo['patientName'].' has '.count($filteredLabs). ' lab results...'
                . '\n\t... '.print_r($filteredLabs, TRUE));
        //END TEMPORARY DIAGNOSTIC LOGGING
...
//            return array('renal' => $renalLabs, 'coagulation' => $coagulationLabs);
        ...
        foreach($sortedLabs as $lab)
        {
            ...
            if($foundCreatinine)
            {
               ...
               $aDiagLabs[] = array('DiagDate'=>$lab['date'], 'Creatinine'=>$value, 'eGFR'=>$eGFR, 'Ref'=>$eGFRSource);
    ...
    function getDoseHxDetail()
    {
        ...
        error_log('WARNING!!!!!! Calling hardcoded demo procedure getDoseHxDetail()!!!!!!!!');
        return array(
