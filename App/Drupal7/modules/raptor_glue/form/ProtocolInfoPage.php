...
class ProtocolInfoPage extends \raptor\ASimpleFormPage
{
    ...
    private function loadExamFieldValues($nSiteID,$nIEN,&$myvalues,$newerthan_dt=NULL)
    {
        ...
        try
        {
            ...
            $msg = 'Unable to load exam equipment radiation dose information for ticket ['.$nSiteID.'-'.$nIEN.'] because '.$ex->getMessage();
            error_log($msg);
            drupal_set_message($msg,'error');
        }
    ...
    private function validateDefaultValueAcknowledgements($myvalues)
    {
        ...
        $sections = array(
            ...
            ,'radioisotope'=>'Radioisotope Section'
 ...
 //drupal_set_message('111 LOOK myvalues at clicked=['.$form_state['clicked_button']['#value'].']');        
        ...
        foreach($aContraindications as $oCI)
        {
            ...
            foreach($aCIS as $oCIS)
            {
                ...
                if(isset($aMapCI_AlreadyAcknowledged[$sRuleName]))
                {
                    ...
                    $blurb = 'Acknowledged on '.$aDetails['acknowledged_dt'].' by '.$aDetails['fullname'];
                    $aCI_AlreadyAcknowledgedMarkup[][] = array('#markup' => "\n<li>\n<a href='javascript:showContraIndicationsExplanationPopup($nRuleExplainID);'>".$oCIS->getMessage()."</a><data-explanation hidden id='$nRuleExplainID'>".$oCIS->getExplanation()
                            ."</data-explanation>"
                            . " ($blurb)"
                            . "</li>");
                } else {
                    ...
                    $nCI_Acknowledge++;
                    $aCI_2Ack['ci_rules'][$sRuleName] = array('#type' => 'hidden', '#value' => $sRuleName);   //So we know what to record in database.
    ...
    function getForm($form, &$form_state, $disabled, $myvalues_override=NULL)
    {
        ...
        $aMapCI_AlreadyAcknowledged = $this->getAllAcknowledgedContraindicationsMap($nSiteID, $nIEN);
        $aCIResultMap = $this->getContraindicationFormMarkup($nSiteID, $nIEN, $myvalues, $protocolValues, $oPSD, $aMapCI_AlreadyAcknowledged);
        ...
        $protocolInputDisable = ($sCWFS !== 'AC') && ($sCWFS !== 'RV') && ($sCWFS !== 'CO');  //20140821
        ...
        //PROTOCOL MODE
        $clues = array();
        $clues['cpt_codes'] = array();  //TODO provide the codes associated with this order
        $clues['modality_abbr'] = $this->m_oUtility->inferModalityFromPhrase($myvalues['procName']);
        $clues['contrast'] = $this->m_oUtility->inferContrastFromPhrase($myvalues['procName']);
        $clues['keywords'] = $this->m_oUtility->getOrderPhraseKeywords($myvalues['procName']);
        ...
        if(count($aAllCIWarnings) > 0) //$sStaticWarningMsgsHTML !== NULL)
        {
            ...
            if($nCI_Acknowledge > 0)
            {
                ...
                $form['data_entry_area2']['contraindication_ra']  = array(
                        '#type'     => 'fieldset',
                        '#title'    => t('Contraindications Requiring Acknowledgement'),
                        '#disabled' => $disabled,
