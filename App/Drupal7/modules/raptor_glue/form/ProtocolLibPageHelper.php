...
//drupal_set_message(">>>>write $radioisotope_enteric_tx and $radioisotope_iv_tx  <br>rawentid={$myvalues['radioisotope_enteric_id']} <br>rawenttx={$myvalues['radioisotope_enteric_customtx']} ");
    ...
    function looksValid($form, $myvalues, $formMode)
    {
        ...
        $checks = array(
            ...
            'RI'=>array('Radioisotope','hasRadioisotopeValues','yn_attribs_ri','C','radioisotope_cd','none'),
        ...
        foreach($checks as $code=>$details)
        {
            ...
            if($checkboxisset)
            {
                ...
                if($hasvalues)
                {
                    form_set_error($catradio, 'Did not declare '.$name.' values but some were provided');
                    $bGood = FALSE;
                }
            }
        }
    ...
    function getForm($formType, $form, &$form_state, $disabled, $myvalues, $containerID=NULL)
    {
        ...
        $form['data_entry_area1']['codemappings'] = array(
            ...
            '#disabled' => $disabled,
          '#description' => t('<p>The codes that this protocol maps into so that RAPTOR can more granularly apply matching order criteria.</p>'),
        );
