 ...
 (function($){
  ...
  $.fn.tablePagination = function(settings) {
    ...
    return this.each(function() {
      ...
      currPageId_top = '#tablePagination_currPage_top';
      rowsPerPageId = '#tablePagination_rowsPerPage';
      rowsPerPageId_top = '#tablePagination_rowsPerPage_top';
      firstPageId = '#tablePagination_firstPage';
      firstPageId_top = '#tablePagination_firstPage_top';
      prevPageId = '#tablePagination_prevPage';
      prevPageId_top = '#tablePagination_prevPage_top';
      nextPageId = '#tablePagination_nextPage';
      nextPageId_top = '#tablePagination_nextPage_top';
      lastPageId = '#tablePagination_lastPage';
      lastPageId_top = '#tablePagination_lastPage_top';
      ...
      function resetCurrentPage(currPageNum) {
        ...
        $(currPageId_top).val(currPageNumber);
      }
    ...
    if (defaults.topNav) {
      ...
      /***** Top Pagination *****/
      // duplicate the bottom pagination

      $(firstPageId_top).bind('click', function (e) { $(firstPageId).click() });
        
      $(prevPageId_top).bind('click', function (e) { $(prevPageId).click(); });

      $(nextPageId_top).bind('click', function (e) { $(nextPageId).click(); });

      $(lastPageId_top).bind('click', function (e) { $(lastPageId).click(); });

      $(currPageId_top).bind('change', function (e) {
      ...
      $(table)[tblLocation]().find(rowsPerPageId).bind('change', function (e) {
        ...
        resetCurrentPage(1);
        //Keep bottom and top pagination in sync.
        $(rowsPerPageId_top).val($(rowsPerPageId).val());
      });

      $(rowsPerPageId_top).bind('change', function(e) {
          //Keep bottom and top pagination in sync.
          $(rowsPerPageId).val($(this).val());
          $(rowsPerPageId).change();
      });
