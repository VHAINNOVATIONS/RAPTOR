...
class ProtocolSupportingData
{
    ...
    function __construct($oContext)
    {
        ...
        if(count($orders) == 0)
        {
            //This should never happen, but if it does help us understand why.
            error_log("Did NOT find any orders for " . print_r($this->m_oContext, true));
            $this->m_aOneOrder = null;
            $this->m_aPatientInfo = null;
        } else {
            $this->m_aOneOrder = $orders[0];
            $this->m_aPatientInfo = $wl->getPatient($this->m_aOneOrder["PatientID"]);
        }
        if($this->m_aPatientInfo == null)
        {
            die('Did NOT get patient data in constructor of ProtocolSupport for ' . print_r($orders, true));
        }
    }
    ...
    {
        return array("RqstBy"=>$this->m_aOneOrder["RqstBy"],
                     "PCP"=>$this->m_aPatientInfo["teamPcpName"],
                     "AtP"=>$this->m_aPatientInfo["teamAttendingName"],
                     "RqstStdy"=>$this->m_aOneOrder["RqstStdy"],
                     "RsnStdy"=>$this->m_aOneOrder["RsnStdy"],
                    );
    }
...
//            );
    ...
    /**
     * @description Returns array of 2 arrays: DisplayVitals and AllVitals
     * @return array
     */
    private function getVitalsData()
    {
		//20140331 FJF updated Date Taken label to match spec
//      EXAMPLE OUTPUT:
//              return array(
//            array("Date Taken"=>"2013-10-10","Temp"=>98.7,"Height"=>123.4,"Weight"=>128,"BMI"=>85,"BloodPrsr"=>"98/67","Pulse"=>99,"Resp"=>123,"Pain"=>98,"C/G"=>"1/2","Pox"=>"asdf","CVP"=>"cjhdjd","BloodGluc"=>83),
//            array("Date Taken"=>"2013-10-15","Temp"=>100,"Height"=>123.4,"Weight"=>118,"BMI"=>65,"BloodPrsr"=>"98/67","Pulse"=>99,"Resp"=>123,"Pain"=>98,"C/G"=>"1/2","Pox"=>"asdf","CVP"=>"cjhdjd","BloodGluc"=>83),
//            array("Date Taken"=>"2013-11-10","Temp"=>100.2,"Height"=>123.4,"Weight"=>97,"BMI"=>55,"BloodPrsr"=>"98/67","Pulse"=>99,"Resp"=>123,"Pain"=>98,"C/G"=>"1/2","Pox"=>"asdf","CVP"=>"cjhdjd","BloodGluc"=>83),
//            );
       //$serviceResponse = $this->m_oContext->getEMRService()->getVitalSigns();
       ...
       if(!isset($serviceResponse->getVitalSignsResult->arrays->TaggedVitalSignSetArray->count))
        ...
        for($i=0; $i<$numTaggedVitals; $i++){
            ...
            for($j=0; $j<$numVitalsTO; $j++){
                ...
                $date = isset($vitalsSetTO->timestamp) ? date("m/d/Y h:i a", strtotime($vitalsSetTO->timestamp)) : " ";
                $name = isset($vital->type->name) ? $vital->type->name : "";
                $value = isset($vital->value1) ? $vital->value1 : "";
                $units = isset($vital->units) ? $vital->units : "";
                $rawTime = isset($vitalsSetTO->timestamp) ? $vitalsSetTO->timestamp : "";
                $allVitals[$k++] = array('date'=>$date, 'name'=>$name, 'value'=>$value, 'units'=>$units, 'rawTime'=>$rawTime);
                if(isset($vital->type->name)){
                    $displayVitals[$i]['Date Taken'] = isset($vitalsSetTO->timestamp) ? date("m/d/Y h:i a", strtotime($vitalsSetTO->timestamp)) : " ";
                    if(strcasecmp('temperature', $vital->type->name) == 0){
                        $displayVitals[$i]['Temp'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('height', $vital->type->name) == 0){
                        $cms = round($vital->value1*2.54, 1);
                        $displayVitals[$i]['Height'] = $vital->value1." ".$units." (".$cms." cm)";
                    }
                    elseif(strcasecmp('weight', $vital->type->name) == 0){
                        $kgs = round($vital->value1*0.45359237, 1);
                        $displayVitals[$i]['Weight'] = $vital->value1." ".$units." (".$kgs." kg)";
                    }
                    elseif(strcasecmp('body mass index', $vital->type->name) == 0){
                        $displayVitals[$i]['BMI'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('blood pressure', $vital->type->name) == 0){
                        $displayVitals[$i]['Blood Pressure'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('pulse', $vital->type->name) == 0){
                        $displayVitals[$i]['Pulse'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('respiration', $vital->type->name) == 0){
                        $displayVitals[$i]['Resp'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('pain', $vital->type->name) == 0){
                        $displayVitals[$i]['Pain'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('c/g', $vital->type->name) == 0){
                        $displayVitals[$i]['C/G'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('Pulse Oxymetry', $vital->type->name) == 0){
                        $displayVitals[$i]['Pox'] = $vital->value1." ".($units == "" ? $defPOXUnits : $units);
                    }
                    elseif(strcasecmp('cvp', $vital->type->name) == 0){
                        $displayVitals[$i]['CVP'] = $vital->value1." ".$units;
                    }
                    elseif(strcasecmp('blood glucose', $vital->type->name) == 0){
                        $displayVitals[$i]['Blood Glucose'] = $vital->value1." ".$units;
                    }
                }
            }
        }
...
//      EXAMPLE OUTPUT:
//              return array(
//            array("DateTaken"=>"2013-10-10","Temp"=>98.7,"Height"=>123.4,"Weight"=>128,"BMI"=>85,"BloodPrsr"=>"98/67","Pulse"=>99,"Resp"=>123,"Pain"=>98,"C/G"=>"1/2","Pox"=>"asdf","CVP"=>"cjhdjd","BloodGluc"=>83),
//            array("DateTaken"=>"2013-10-15","Temp"=>100,"Height"=>123.4,"Weight"=>118,"BMI"=>65,"BloodPrsr"=>"98/67","Pulse"=>99,"Resp"=>123,"Pain"=>98,"C/G"=>"1/2","Pox"=>"asdf","CVP"=>"cjhdjd","BloodGluc"=>83),
//            array("DateTaken"=>"2013-11-10","Temp"=>100.2,"Height"=>123.4,"Weight"=>97,"BMI"=>55,"BloodPrsr"=>"98/67","Pulse"=>99,"Resp"=>123,"Pain"=>98,"C/G"=>"1/2","Pox"=>"asdf","CVP"=>"cjhdjd","BloodGluc"=>83),
//            );
...
//      EXAMPLE OUTPUT:
//        return array(
//          "Temperature"=>array("1/1/2014","98.6 F"),
//          "Heart Rate"=>array("1/2/2014","None Found"),
//          "Blood Pressure"=>array("1/1/2014","134/81 mmHg"),
//          "Height"=>array("1/2/2014","71 in (180.3 cm)"),
//          "Weight"=>array("1/1/2014","178 lb (80.7 kg)"),
//          "Body Mass Index"=>array("1/2/2014","25"),
//        );
        $result = array();
...
*/          if(in_array(strtoupper($vital['name']), $tempLabels)){ // Temp
    ...
    function getAllergiesDetail()
    {
        ...
        //return array("Allergies"=>$allergies, "DisplayAllergies"=>$displayAllergies, "AllergySummary"=>$allergySummary); 
        return $displayAllergies;
    }
...
//      EXAMPLE OUTPUT:         
//        return array(
//            array("ProcDate"=>"2013-10-10","PLT"=>"abc","PT"=>"asdf","INR"=>"asdf","PTT"=>"eueu","HCT"=>"83838","Ref"=>"uoaeuiosdaf"),
//            array("ProcDate"=>"2013-11-10","PLT"=>"abc","PT"=>"asdf","INR"=>"asdf","PTT"=>"eueu","HCT"=>"83838","Ref"=>"uoaeuiosdaf"),
//            array("ProcDate"=>"2013-12-10","PLT"=>"abc","PT"=>"asdf","INR"=>"asdf","PTT"=>"eueu","HCT"=>"83838","Ref"=>"uoaeuiosdaf"),
//            );
        $wl = new WorklistData($this->m_oContext); //FJF 20140331 to prevent error
        
        //$orders = $wl->getWorklistForProtocolSupport();
        //$order = $orders[2];   //???
        
        $proc = strtoupper($this->m_aOneOrder['RqstStdy']); // Procedure is alialsed as RqstdStdy in method of WorklistData object
        //do not echo --- else shows up on page! echo $proc;
        $isProc = TRUE; //strpos($proc, "PROCEDURE") !== FALSE;
        $isDiag = FALSE; //strpos($proc, "PROCEDURE") !== FALSE; //TODO compute IsDiagnostic correctly - need to figure out how

        $nMAX_NUMBER_ROWS=999;  //No filter for labs tab as per DrM repeated request on 3/7/2012

        // Get Patient Information
        $patientInfo = $this->m_aPatientInfo;   // $wl->getPatient($order["PatientID"]);
            ...
            if($isDiag){
                if($foundCreatinine){
                    if ($nCreatinine++ < $nMAX_NUMBER_ROWS){
                       ...
                       $filteredLabs[] = array($lab['date'], $value, round($eGFR,0).$eGFRUnits.$eGFRLabel, $lab['refRange']);
                    }
                }
            }
            elseif ($isProc){
...
//        );
        ...
        $isDiag = false;    //$oContext->getProcedure()->isDiagnostic();
...
//            return array('renal' => $renalLabs, 'coagulation' => $coagulationLabs);
        ...
        foreach($sortedLabs as $lab)
        {
            ...
            if($foundCreatinine)
            {
                if ($nCreatinine++ < 3)
                {
                  ...
                  $result[] = array('DiagDate'=>$lab['date'], 'Creatinine'=>$value, 'eGFR'=>$eGFR, 'Ref'=>$eGFRSource);
                  return $result;
               }
            }
        }
     ...
     * @return type array of arrays
     */
    function getDoseHxDetail()
    {
        return array(
            array("Action"=>"action1","ExamDate"=>"2013-12-15","ProcType"=>"asdflksdlkf","Height"=>123.5,"Weight"=>111,"BMI"=>99,"CTDIvol"=>123,"DLP"=>32,"RadTrcDose"=>64,"EntryState"=>"asdf"),
            array("Action"=>"action2","ExamDate"=>"2013-12-15","ProcType"=>"asdflksdlkf","Height"=>123.5,"Weight"=>111,"BMI"=>99,"CTDIvol"=>123,"DLP"=>32,"RadTrcDose"=>64,"EntryState"=>"asdf"),
        );
    }
...
//                $tempRpt['guid'] = $guid;
                ...
                $tempRpt['specimenCollectionDate'] = isset($RptTO->specimen->collectionDate) ? $RptTO->specimen->collectionDate : " ";
