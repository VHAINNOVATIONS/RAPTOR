...
/**
 * Implementes the schedule ticket page.
 *
 * @author FrankWin7VM
 */
class ScheduleTicketPage
{
    ...
    function updateDatabase($form, $myvalues)
    {
        ...
        drupal_set_message('Scheduled ' . $myvalues['tid'] . ' (' . $myvalues['procName'] .')' . $sforinfo); 
    ...
    public function getForm($form, &$form_state, $disabled, $myvalues)
    {
        ...
        $form['data_entry_area1']['toppart']['heading'] = array(
            '#markup' => '<table id="raptor-schedule-table"><tbody>'
                . '<tr><th>Tracking ID</th><td>' . $myvalues['tid'] . '</td>'
                . '<th>Patient Name</th><td>' . $myvalues['PatientName']  . '</td></tr>'
                . '<tr><th>Study</th><td>' . $myvalues['procName']  . '</td>'
                . '<th>Patient DOB</th><td>' . $myvalues['PatientDOB'] . '</td></tr>'
                . '<tr><th>Requested Date</th><td>' . $myvalues['RequestedDate'] . '</td>'
                . '<th>Urgency</th><td>' . $myvalues['Urgency'] . '</td></tr>'
                .'</tbody></table>',
        );
        ...
        if($disabled || !is_array($myvalues['rooms']))
        {
            ...
            {
                $htmldurations .= '<li><span class="location-option"><a href="javascript:setTextboxByName(' 
                        . "'location_tx','" . $aRoom[0] . "' )" . '"'
        ...
        if($disabled || !is_array($myvalues['durations']))
        {
            
        } else {
            $htmldurations = '<ul>';
            $aRooms = $myvalues['durations'];
            foreach($aRooms as $aRoom)
            {
                $htmldurations .= '<li><span class="location-option"><a href="javascript:setTextboxByName(' 
                        . "'duration_am','" . $aRoom . "' )" . '"'
                        . ' title="'.$aRoom.' minutes" >'.$aRoom.'</a></span>';
            }
            $htmldurations .= '</ul>';
        ...
        $form['data_entry_area1']['middlepart']['canceled_reason_tx'] = array(
            "#type" => "select",
            "#title" => t("Reason for cancelation (If canceled)"),
            "#options" => array(
                '' => t(''),
        ...
        $form['data_entry_area1']['middlepart']['suggested_uid'] = array(
            "#type" => "select",
            "#title" => t("Attention of suggestion"),
            "#options" => $options,
