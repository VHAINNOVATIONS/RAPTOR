module.exports = {
	...
	checkDcAndSignParams: function(params) {
		if (!params.hasOwnProperty("orderId") || params.orderId == "") {
			throw new Error("No orderId");
		}
		if (!params.hasOwnProperty("providerId") || params.providerId == "") {
			throw new Error("No providerId");
		}
		if (!params.hasOwnProperty("locationId") || params.locationId == "") {
			throw new Error("No locationId");
		}
		if (!params.hasOwnProperty("reasonId") || params.reasonId == "") {
	...
	validateOrderActionNature: function(params, session, ewd) {
		...
		params.rpcArgs = [{ type: "LITERAL", value: params.providerId }];
	...
	saveOrderChecksFromReleaseOrderResponse: function(params, session, ewd) {
		if (params.rawResponse == "") {
			return true;
		}
		
		var orderCheckLines = params.rawResponse.split("\r\n");
		
		params.rpcName = "ORWDXC SAVECHK";
		params.rpcArgs = [];
		params.rpcArgs.push({ type: "LITERAL", value: params.patientId });
		params.rpcArgs.push({ type: "LITERAL", value: params.overrideReason || "" });
		
		var list = { "ORCHECKS" : {} };
		for (var i = 0; i < orderCheckLines.length; i++) {
			list.ORCHECKS[(i+1).toString()] = orderCheckLines[i];
		}
		params.rpcArgs.push({ type: "LIST", value: list });
	...
	signOrder: function(params, session, ewd) {
		...
		params.rpcArgs.push({ type: "LITERAL", value: vistaLib.encryptRpcParameter(params.eSig, session, ewd) });
		...
		var arg = { "1": (params.orderId + "^" + signOnChartFlag + "^1^" + natureOfOrder) }; // e.g. 21354;1^0^1^E
	...
	cancelOrder: function(params, session, ewd) {
		...
		params.rpcArgs.push({ type: "LITERAL", value: params.providerId });
		params.rpcArgs.push({ type: "LITERAL", value: params.clinicId });
		params.rpcArgs.push({ type: "LITERAL", value: params.reasonId });
	...
	getRadiologyOrderChecksForAcceptOrderRequest: function(params, session, ewd) {
		...
		params.rpcArgs.push({type: "LITERAL", value: params.locationId});
		params.rpcArgs.push({type: "LIST", value: { "1":params.orderableItemId } });
	...
	saveNewRadiologyOrderWithRules: function(params, session, ewd) {
		...
		if (params.eSig && params.eSig != "") {
			if (!vistaLib.isValidESig(params.eSig, session, ewd)) {
		...
		var newOrder = saveNewRadiologyOrder(params, session, ewd);
		...
		if (params.eSig && params.eSig != "") {
			...
			if (orderActionValidationText != "OK") {
				this.unlockOrder(params, session, ewd);
				this.unlockOrdersForPatient(params, session, ewd);
			...
			//if (
			// save order checks from response
	...
	saveNewRadiologyOrder: function(params, session, ewd) {
		...
		if (params.modifiers && params.modifiers.length > 0) {
			...
			for (var i = 0; i < params.modifiers.lenght; i++) {
		...
		var orderStatusDict = this.getOrderStatuses(session, ewd);
		return this.toOrder(vistaLib.runRpcForCreateOrder(params, session, ewd), orderStatusDict);
	}
