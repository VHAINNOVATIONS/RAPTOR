...
var operations = {
  ...
  getDashboardDetailsMap: {
	...
	gvv: {
		GET: function(ewd, session) {
			var ok = ewd.util.restoreSymbolTable(ewd, session);	//Flush symbol table and replace with ours
			var result = vista.getVariableValue(ewd.query.arg, session, ewd);
			ok = ewd.util.saveSymbolTable(ewd, session);	//Grab our symbol table for use next time
			return result;
		}
	},
	...
	cancelRadiologyOrder: {
		GET: function(ewd, session) {
			...
			//vista.setContext("OR CPRS GUI CHART", session, ewd);
			var ok = ewd.util.restoreSymbolTable(ewd, session);	//Flush symbol table and replace with ours
	...
	scratch: {
		GET: function(ewd, session) {
			var params = {};
			for (var v in ewd.query) {
				if (ewd.query.hasOwnProperty(v)) {
					params[v] = ewd.query[v];
				}
			}H
			
			//var params = { patientId: ewd.query.patientId };
			var ok = ewd.util.restoreSymbolTable(ewd, session);	//Flush symbol table and replace with ours
			var result = vista.scratch(params, session, ewd);
			ok = ewd.util.saveSymbolTable(ewd, session);	//Grab our symbol table for use next time
			return result;
		}
	},
	...
	loginRpc: {
		GET: function(ewd, session) {
			var params = {
				accessCode: "7radiologist",
				verifyCode: "radiology"
			};
			ewd.util.clearSymbolTable(ewd, session);
			var result = vista.login2(params, session, ewd);
			ok = ewd.util.saveSymbolTable(ewd, session);	//Grab our symbol table for use next time
			return { login: result, sst: ok };
		}
	},
...
module.exports = {
  ...
  parse: function(ewd) {
    ...
    if (resource !== 'initiate') {
      ...
      if (resource !== 'login' && resource !== 'loginRpc') {
