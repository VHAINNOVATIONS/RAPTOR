  ...
  getDashboardDetailsMap: {
	...
	cancelRadiologyOrder: {
		GET: function(ewd, session) {
			var params = {
				...
				userDuz: ewd.query.userId,
				providerDuz: ewd.query.providerDuz,
    ...
    getRadiologyOrderChecks: {
		GET: function(ewd, session) {
			var params = {
				patientId: ewd.query.patientId,
				orderStartDateTime: ewd.query.orderStartDateTime,
				locationId: ewd.query.locationId,
				orderableItem: ewd.query.orderableItem
			};
			var ok = ewd.util.restoreSymbolTable(ewd, session);	//Flush symbol table and replace with ours
    ...
    getUserSecurityKeys: {
		GET: function(ewd, session) {
			var ok = ewd.util.restoreSymbolTable(ewd, session);	//Flush symbol table and replace with ours
			var result = vista.getUserSecurityKeys({ uid: ewd.query.uid }, session, ewd);
			ok = ewd.util.saveSymbolTable(ewd, session);	//Grab our symbol table for use next time
			return result;
		}
    },
    ...
    validateEsig: {
		GET: function(ewd, session) {
			var ok = ewd.util.restoreSymbolTable(ewd, session);	//Flush symbol table and replace with ours
			var result = vista.isValidESig({ eSig: ewd.query.eSig }, session, ewd);
			ok = ewd.util.saveSymbolTable(ewd, session);	//Grab our symbol table for use next time
			return result;
		}
    },
